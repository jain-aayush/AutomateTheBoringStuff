"""
Write a program to read in the contents of several text files 
(you can make the text files yourself) and insert those contents into a spreadsheet, 
with one line of text per row.
The lines of the first text file will be in the cells of column A, 
the lines of the second text file will be in the cells of column B, and so on.
"""

import os, openpyxl

wb = openpyxl.Workbook()
sheet = wb.active

filelocation = input("Enter the place where your text files are located\n")

col = 1

for folderName, subfolders, filenames in os.walk(filelocation):
    for filename in filenames:
        if(filename.endswith('.txt')):
            with open(os.path.join(folderName, filename), 'r') as file:
                fileContent = file.readlines()    
            row = 1
            for line in fileContent:
                print(line, row)
                sheet.cell(row=row, column=col).value = line
                row+=1
            col+=1

wb.save("Filecontents.xlsx")